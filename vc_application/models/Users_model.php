	 ...
	 */	function validate($user_name, $password)
	{
		...
		if (count($query->result_array()) == 1) {
			...
			foreach ($query->result() as $row) {
				...
				if ($row->image == '') {
					$return['cust_img'] = base_url() . 'assets/images/man-person.png';
				} else {
					$return['cust_img'] = base_url() . 'images/user/' . $row->image;
				}
			}
			return $return;
		} else {
...
Team Wishzon");
	...
	function add_user_package($data_to_store)
	{
		$this->db->insert('package_purchase', $data_to_store);
		return TRUE;
	}

	function get_payment_amount($id)
	{
	...
	{
		$query = $this->db->query('SELECT * FROM installment where user_id = ' . $id . ' and status = "Paid"');
		return $query->num_rows();
	}

	function get_installments_remaining($id)
	{
		$query = $this->db->query('SELECT * FROM installment where user_id = ' . $id . ' and status = "Active"');
		return $query->num_rows();
	}
