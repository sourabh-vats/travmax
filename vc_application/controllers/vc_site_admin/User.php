		...
		/* start **/
				$placement_id = array();
					...
					while($p < 1) {
						...
						if(!empty($my_team) && $my_team[0]['macro']>0) { $matrix = 33; } else { $matrix = 11; } 
						if(!empty($my_team) && count($my_team) >= $matrix) {
							foreach($my_team as $team) {
								$directs = $this->Users_model->show_directs($team['customer_id']);
								if($team['macro']>0) { $matrix = $team['macro']; } else { $matrix = 11; } 
								if(count($directs) < $matrix) {
									$placement_id = $team['customer_id']; $p++;
									break;
								}
							}
							if(empty($placement_id)) {
